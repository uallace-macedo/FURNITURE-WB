      ...
      <div className="container mx-auto flex justify-center lg:justify-end">
        <div className='w-full max-w-[558px] text-center text-white lg:text-left'>
          ...
          <div className='flex flex-col lg:flex-row gap-[22px]'>
            <input 
              className='h-[60px] px-6 bg-white outline-none placeholder:text-gray-400 text-gray-600 rounded-lg lg:flex-1/2'
              type="text"
              placeholder={placeholder}
            />
            <button className='bg-primary h-[60px] hover:bg-slate-900 px-7 rounded-lg font-medium text-xl transition cursor-pointer'>{buttonText}</button>
          </div>
        </div>
      </div>
    </section>
